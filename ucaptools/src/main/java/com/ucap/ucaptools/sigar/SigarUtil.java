package com.ucap.ucaptools.sigar;
 ...
 * @Description: Sigar获取用户相关系统
 * @author 0000
 * @date 2014年6月8日 下午10:55:09
 * @version V1.0
 */
@Component("systemInfo")
public class SigarUtil implements SystemInfo {

	private static SigarUtil sigarUtil = null;
	...
	public Map<String, Object> getMemory() {
		...
		try {
			...
			// 内存总量
			result.put("total", mem.getTotal() / 1024L);
			// 当前内存使用量
			result.put("used", mem.getUsed() / 1024L);
			// 当前内存剩余量
			result.put("free", mem.getFree() / 1024L);
			Swap swap = sigar.getSwap();
			// 交换区总量
			result.put("swaptotal", swap.getTotal() / 1024L);
			// 当前交换区使用量
			result.put("swapused", swap.getUsed() / 1024L);
			// 当前交换区剩余量
			result.put("swapfree", swap.getFree() / 1024L);
		} catch (SigarException e) {
			e.printStackTrace();
		}
		return result;
