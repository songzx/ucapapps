...
var App = function() {
	...
	var handleResponsiveOnResize = function() {
		/*
		 * var resize; if (isIE8) { var currheight; $(window).resize(function() {
		 * if(currheight == document.documentElement.clientHeight) { return;
		 * //quite event since only body resized not window. } if (resize) {
		 * clearTimeout(resize); } resize = setTimeout(function() {
		 * handleResponsive(); }, 50); // wait 50ms until window resize
		 * finishes. currheight = document.documentElement.clientHeight; //
		 * store last body client height }); } else {
		 * $(window).resize(function() { if (resize) { clearTimeout(resize); }
		 * resize = setTimeout(function() { console.log('resize');
		 * handleResponsive(); }, 50); // wait 50ms until window resize
		 * finishes. }); }
		 */
	}
	...
	var handleSidebarToggler = function() {
		...
		/*
		 * $('.page-sidebar').on('keypress', '.sidebar-search input', function
		 * (e) { if (e.which == 13) { window.location.href =
		 * "extra_search.html"; return false; //<---- Add this line } });
		 */
		 ...
		 * e.preventDefault();
		 * 
		 * if ($('body').hasClass("page-sidebar-closed")) { if
		 * ($('.sidebar-search').hasClass('open') == false) { if
		 * ($('.page-sidebar-fixed').size() === 1) { $('.page-sidebar
		 * .sidebar-toggler').click(); //trigger sidebar toggle button }
		 * $('.sidebar-search').addClass("open"); } else { window.location.href =
		 * "extra_search"; } } else { window.location.href =
		 * "extra_search.html"; } });
		 */
	}
	...
	var handleHorizontalMenu = function() {
		 ...
		 * window.location.href = "extra_search.html"; e.preventDefault(); });
		 */

		// handle hor menu search form on enter press
		/*
		 * $('.header').on('keypress', '.hor-menu .search-form input', function
		 * (e) { if (e.which == 13) { window.location.href =
		 * "extra_search.html"; return false; } });
		 */
	}
	...
	var handlePortletTools = function() {
		jQuery('body').on(
				'click',
				'.portlet .tools a.remove',
				function(e) {
					e.preventDefault();
					var removable = jQuery(this).parents(".portlet");
					if (removable.next().hasClass('portlet')
							|| removable.prev().hasClass('portlet')) {
